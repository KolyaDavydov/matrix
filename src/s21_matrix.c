...
/**
 * @brief транспонирование матрицы (замена строк матрицы столбцами с сохранением их номеров)
 *
 * @param A указатель на матрицу
 * 
...
int s21_calc_complements(matrix_t *A, matrix_t *result) {
  ...
  if (A->matrix != NULL && A != NULL && A->columns >= 1 && A->rows >= 1) {
    if (A->columns == A->rows) {
        ...
        if (error == OK) {
          for (int i = 0; i < result->rows; i++) {
            for(int j = 0; j < result->columns; j++) {
              ...
              result->matrix[i][j] = s12_minor_calculation(&tmp) * pow(-1, i + j);
 ...
 */
void s21_create_minor_matrix_for_element(int num_row, int num_column, matrix_t *A, matrix_t *result) {
  int error = OK;
  if (A->matrix != NULL && A != NULL && A->columns >= 1 && A->rows >= 1) {
      ...
      for (int i = 0; i < A->rows; i++) {
        for (int j = 0; j < A->columns; j++) {
          if (i != num_row && j != num_column) {
            ...
            if (result_column == result->columns) result_row++, result_column = 0;
          }
        }
      }
    }
